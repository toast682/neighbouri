...
export default function ChatsListScreen(props) {
  ...
  async function getUserChats() {
    ...
    await firestore()
      ...
      .get()
      .then((chatDocs) => {
            const chats = chatDocs.docs.map(d => d.data());
            const userchts = chats.filter(ch => {
                return ch.chatMembers.includes(currentUserId);
            });
            const latestMsgs = userchts.map(ch => ch.messages.pop());
  ...
  async function getLatestMessengerImage() {
  ...
  async function buildObject(doc) {
    ...
    const latestMsgsWPic = latestChatMessages.map(msg => {
  ...
  function buildCreatedAtDateString(msgObject) {
    ...
    const hourMin = date.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' }).split(":");
    const monthDay = date.toLocaleDateString('en-US', { month: 'short', day: 'numeric'}).split(',')[0];
    let isNow = false;
    const now = new Date();
    ...
    const dateTimeString = hourMin[0] + ":" + hourMin[1] + ' PM ' + '(' + monthDayOrNow + ')';
    ...
    <View style={styles.container}>
      <FlatList
        ...
        renderItem={({item}) => {
          ...
          return (
            <TouchableOpacity
            onPress={() => {console.log('nativigation.nativigate into chat')}}
            style={{
              alignSelf: 'stretch',
              borderColor: "#E5E5E5",
              borderBottomWidth: 2,
              ...
              <View style={{
                ...
                <View style={{flexDirection: 'row'}}>
                    ...
                    <View style={{
                        ...
                        <Text style={{fontWeight: 'bold'}}>{item && item.user.name}</Text>
